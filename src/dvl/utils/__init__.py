...
class QwenVL(ModelConfig):
    def __init__(
        ...
        if model_size <= 14:
            ...
            print("Unknown model_id: {}. Setting tensor_parallel_size to 1....".format(model_id))
            self.default_engine_args["tensor_parallel_size"] = 1
        ...
        print(f"video parameters: max_num_frames={self.max_num_frames}, video_min_pixels={self.video_min_pixels}, video_max_pixels={self.video_max_pixels}")
    ...
    def get_prompt_from_question(self, messages: List[Dict]):
        ...
        if messages_contain_video(messages):
            ...
            for i, video_inp in enumerate(video_inputs):
                if isinstance(video_inp, list):
                    video_inp = np.concatenate([np.array(frame) if isinstance(frame, Image.Image) else frame for frame in video_inp], axis=0)
                    video_inputs[i] = video_inp
...
class InternVL3HF(ModelConfig):
    def __init__(
            self, model_id: str, max_model_len: int = None, max_tokens: int = None, max_num_frames: int = 32,
    ):
        self.processor = AutoProcessor.from_pretrained(model_id)
        self.default_engine_args["model"] = model_id
        self.default_engine_args["config_format"] = "hf"
    ...
    def __init__(self, model_id: str, max_model_len: int = None, max_tokens: int = None, max_num_frames: int = 32):
        ...
        if self.tokenizer.chat_template is None:
            if model_id != "llava-hf/llava-1.5-7b-hf":
                raise NotImplementedError(f"If tokenizer.chat_template is None, it should be llava-hf/llava-1.5-7b-hf but got {model_id}")
...
def build_model_config(model_id: str, **kwargs) -> ModelConfig:
    if "llava" in model_id.lower():
        ...
        if "hf" in model_id.lower():
            model_config =  InternVL3HF(model_id=model_id, **kwargs)
        else:
